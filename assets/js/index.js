...
let weatherApiKey = "&appid=d025108d5b5bff4e0d318b8b71ba2019";
...
function renderCurrentWeather(coordinatesData, openWeatherData) {
    // sets card content
    ...
    mainCardTemp.textContent = `${convertToFahrenheitToCelsius(
...
function renderForecast(openWeatherData) {
    for (let i = 0; i < forecastCard.length; i++) {
        ...
        forecastCardTemp[i].textContent = `${convertToFahrenheitToCelsius(
...
function appendToSearchHistory(weatherData) {
